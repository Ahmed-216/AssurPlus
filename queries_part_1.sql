...
WITH campaign_sequence AS (
    SELECT
        ...
        campagne_nom,
        LAG(campagne_id::INTEGER) OVER (PARTITION BY lead_id ORDER BY date_appel::TIMESTAMP) AS prev_campagne_id,
        LAG(campagne_nom) OVER (PARTITION BY lead_id ORDER BY date_appel::TIMESTAMP) AS prev_campagne_nom,
        LAG(date_appel::TIMESTAMP) OVER (PARTITION BY lead_id ORDER BY date_appel::TIMESTAMP) AS prev_date_appel
    FROM raw.appels
)
SELECT
    lead_id,
    date_appel,
    campagne_id AS current_campagne_id,
    campagne_nom AS current_campagne,
    prev_campagne_id AS previous_campagne_id,
    prev_campagne_nom AS previous_campagne,
    prev_date_appel AS previous_date_appel
FROM campaign_sequence
WHERE prev_campagne_id IS NOT NULL
  AND campagne_id < prev_campagne_id  -- Seulement les régressions
ORDER BY lead_id, date_appel;
